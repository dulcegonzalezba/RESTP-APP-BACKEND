...
export class MockDataService {
  ...
  getUsersByRestaurante(restauranteId: string): Usuario[] {
    return this.usuarios.filter(user => 
      user.restauranteId === restauranteId && user.activo
    );
  }

  getUsersBySucursal(sucursalId: string): Usuario[] {
    return this.usuarios.filter(user => 
      user.sucursalId === sucursalId && user.activo
    );
  }
  ...
  getSucursalesByRestaurante(restauranteId: string): Sucursal[] {
    return this.sucursales.filter(suc => 
      suc.restauranteId === restauranteId && suc.activo
    );
  }
  ...
  canAccessSucursal(user: Usuario, sucursalId: string): boolean {
    ...
    if (user.rol === Role.RESTAURANTE_ADMIN && user.restauranteId === sucursal.restauranteId) {
      return true;
    }
